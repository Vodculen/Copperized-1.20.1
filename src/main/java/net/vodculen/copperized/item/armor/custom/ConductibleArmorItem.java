...
public class ConductibleArmorItem extends ArmorItem {
	private final ArmorMaterial baseMaterial;
	private final ArmorMaterial chargedMaterial;
	private final Type type;

	public ConductibleArmorItem(ArmorMaterial baseMaterial, ArmorMaterial chargedMaterial, Type type, Settings settings) {
	...
	public void inventoryTick(ItemStack stack, World world, Entity entity, int slot, boolean selected) {
		...
		if (!world.isClient && world.isRaining() && !world.isThundering()) {
			if (Math.random() < 0.0005) {
				((ServerWorld) world).setWeather(0, Ticks.getMinutes(5), true, true);
				System.out.println("Started to thunder!");
			}
		}
	}
